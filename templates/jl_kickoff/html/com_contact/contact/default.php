...
<div class="com-contact contact" itemscope itemtype="https://schema.org/Person">
	...
	<?php if ($tparams->get('show_contact_list') && count($this->contacts) > 1) : ?>
		<form action="#" method="get" name="selectForm" id="selectForm">
			...
			<?php echo HTMLHelper::_(
				...
				$this->contacts,
				'select_contact',
				'class="jl-select" onchange="document.location.href = this.value"', 'link', 'name', $this->item->link);
			?>
		</form>
	<?php endif; ?>
	...
	<?php if (($this->params->get('show_info', 1)) and (($this->item->con_position && $tparams->get('show_position')) or ((($this->params->get('address_check') > 0) &&
		($this->item->address || $this->item->suburb  || $this->item->state || $this->item->country || $this->item->postcode))) or ($tparams->get('allow_vcard')) or ($this->item->misc && $tparams->get('show_misc')) )) : ?>
		...
		<div class="jl-width-1-3@l">
			<div class="com-contact__container">
				...
				<div class="jl-card jl-card-default">
					...
					<?php if (($this->item->con_position && $tparams->get('show_position')) or ((($this->params->get('address_check') > 0) &&
						($this->item->address || $this->item->suburb  || $this->item->state || $this->item->country || $this->item->postcode))) or ($tparams->get('allow_vcard')) or ($this->item->misc && $tparams->get('show_misc')) ) : ?>

					<div class="jl-card-body jl-card-small jl-margin-remove-first-child">
						<?php echo '<h3 class="el-title jl-h5 jl-margin-top">' . Text::_('COM_CONTACT_DETAILS') . '</h3>'; ?>
						...
						<?php if ((($this->params->get('address_check') > 0) && ($this->item->address || $this->item->suburb  || $this->item->state || $this->item->country || $this->item->postcode)) or ($tparams->get('allow_vcard')) ) : ?>
	...
	<?php endif; ?>
		...
		<?php if ($tparams->get('show_email_form') && ($this->item->email_to || $this->item->user_id)) : ?>
			<div class="com-contact__form contact-form jl-background-muted jl-padding jl-margin-remove-first-child">
				<?php echo '<h3 class="el-title jl-h3 jl-margin-remove-bottom">' . Text::_('COM_CONTACT_EMAIL_FORM') . '</h3>'; ?>
